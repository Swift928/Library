class Book {
    ...
    info() {
        return `"${this.title}" by ${capitalFirstWord(this.author)}, has ${
            this.pages
        } pages, and I ${this.read ? '' : 'have not '}read it.`;
    }
}
...
class LibraryManager {
    ...
    updateDisplay() {
        this.myLibrary.forEach((book) => {
            ...
            const gridItem = document
                .querySelector('.book-grid-template')
                .cloneNode(true);
            gridItem.classList.remove('book-grid-template');
            gridItem.book = book;

            // Select the elements within the gridItem that will be updated
            gridItem.querySelector('.book-title').textContent =
                capitalFirstWord(book.title);
            gridItem.querySelector('.author').textContent = capitalFirstWord(
                book.author
            );
            ...
            gridItem.addEventListener('click', (e) => {
                if (
                    gridItem.book &&
                    !e.target.closest('.book-buttons-container')
                ) {
    ...
    removeFromLibrary(bookTitle) {
        ...
        let bookIndex = this.myLibrary.findIndex(
